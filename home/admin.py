...
from .models import StudentDetail, AdminProfile,RegisteredStudent
...
# admin.site.site_title = "Student Counseling Admin Portal"
# admin.site.site_header = "Welcome to the Student Counseling Admin Panel"
# admin.site.index_title = "Manage Counseling Activities"
...
# exclude_fields = ('password','id','first_name', 'last_name','last_login','superuser status')
# user_fields = [field.name for field in Students._meta.fields if field.name not in exclude_fields]
# student_fields = [field.name for field in StudentDetail._meta.fields if field.name not in ('id',)]
...
#         allotted = StudentDetail.objects.filter(is_allotted=True).count()
#         not_allotted = StudentDetail.objects.filter(is_allotted=False).count()
#         students = StudentDetail.objects.all()
...
#     def mass_allocation(self, request):
#         StudentDetail.objects.filter(is_allotted=False).update(is_allotted=True)
#         self.message_user(request, "Mass allocation completed successfully.")
#         return redirect("admin:index")

#     def mark_fees(self, request):
#         ids = request.POST.getlist("student_ids")
#         StudentDetail.objects.filter(id__in=ids).update(fees_paid=True)
#         self.message_user(request, "Selected students marked as fees paid.")
#         return redirect("admin:index")
